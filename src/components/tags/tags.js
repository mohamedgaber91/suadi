...
function Tag(props) {
  ...
  const { closable, onClose, color, checked, onChange, data, hottags, animate, children, classes } = props;
  ...
  return checked ? (
    ...
    <TagStyle closable={closable} onClose={log} className={classes} color={color}>
      {children}
    </TagStyle>
  );
}
...
function AnimatedTags(props) {
  ...
  const forMap = (tag) => {
    ...
    return (
      <span key={tag} className="inline-flex items-center text-11 font-semibold [&>span>span>svg]:text-dark">
        {tagElem}
      </span>
    );
  };
  ...
  return (
    <div>
      ...
      {!inputVisible && (
        <TagStyle
          onClick={showInput}
          className="inline-flex items-center gap-1 h-[22px] px-[9px] font-semibold text-11 [&>span>svg]:text-dark"
        >
