...
const CarouselStyleWraper = Styled.div`
  ...
  .ant-carousel .slick-slide h3 {
    color: ${({ theme }) => theme[theme.mainContent]['text-white']};
  }
`;
...
const AvatarWraperStyle = Styled.div`
  ...
  .ant-badge-count{
    ...
    font-weight: 600;
    line-height: 14px;
    border: 3px solid  ${({ theme }) => theme[theme.mainContent]['text-white']};
  }
  .ant-badge-dot{
    top: 0;
    ${({ theme }) => (theme.rtl ? 'left' : 'right')}: 0 !important;
    width: 10px;
    height: 10px;
    border: 2px solid ${({ theme }) => theme[theme.mainContent]['text-white']};
  }
  .btn-outlined{
    padding: 0px 8.05px;
    border-color: ${({ theme }) => theme[theme.mainContent]['border-color-secondary']};
...
`;
const BreadcrumbGlobal = Styled.div`
  .ant-breadcrumb{
    li{
      display:flex;
      align-item:center;
